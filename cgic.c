...
void fcgiInit() ;
void fcgiUninit() ;
int fcgiMain(FCGX_Stream *in,FCGX_Stream *out,FCGX_Stream *err,FCGX_ParamArray envp);
...
static void cgiGetenv(char **s, const char *var,FCGX_ParamArray envp);
...
int cgic_main(FCGX_Stream *in,FCGX_Stream *out,FCGX_Stream *err,FCGX_ParamArray envp) {
	...
	char *e;
	cgiSetupConstants();
	cgiGetenv(&cgiServerSoftware, "SERVER_SOFTWARE",envp);
	cgiGetenv(&cgiServerName, "SERVER_NAME",envp);
	cgiGetenv(&cgiGatewayInterface, "GATEWAY_INTERFACE",envp);
	cgiGetenv(&cgiServerProtocol, "SERVER_PROTOCOL",envp);
	cgiGetenv(&cgiServerPort, "SERVER_PORT",envp);
	cgiGetenv(&cgiRequestMethod, "REQUEST_METHOD",envp);
	cgiGetenv(&cgiPathInfo, "PATH_INFO",envp);
	cgiGetenv(&cgiPathTranslated, "PATH_TRANSLATED",envp);
	cgiGetenv(&cgiScriptName, "SCRIPT_NAME",envp);
	cgiGetenv(&cgiQueryString, "QUERY_STRING",envp);
	cgiGetenv(&cgiRemoteHost, "REMOTE_HOST",envp);
	cgiGetenv(&cgiRemoteAddr, "REMOTE_ADDR",envp);
	cgiGetenv(&cgiAuthType, "AUTH_TYPE",envp);
	cgiGetenv(&cgiRemoteUser, "REMOTE_USER",envp);
	cgiGetenv(&cgiRemoteIdent, "REMOTE_IDENT",envp);
	/* 2.0: the content type string needs to be parsed and modified, so
		copy it to a buffer. */
	e = FCGX_GetParam("CONTENT_TYPE",envp);
	if (e) {
	...
	cgiGetenv(&cgiContentLengthString, "CONTENT_LENGTH",envp);
	cgiContentLength = atoi(cgiContentLengthString);	
	cgiGetenv(&cgiAccept, "HTTP_ACCEPT",envp);
	cgiGetenv(&cgiUserAgent, "HTTP_USER_AGENT",envp);
	cgiGetenv(&cgiReferrer, "HTTP_REFERER",envp);
	cgiGetenv(&cgiCookie, "HTTP_COOKIE",envp);
#ifdef CGICDEBUG
	CGICDEBUGSTART
	fprintf(dout, "%d\n", cgiContentLength);
	fprintf(dout, "%s\n", cgiRequestMethod);
